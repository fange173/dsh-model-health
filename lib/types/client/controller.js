...
function parseStatusView(value) {
    if (value === null || typeof value !== 'object')
        throw new Error('malformed status view');
    const body = value;
    if (body.snapshot === null || typeof body.snapshot !== 'object')
        throw new Error('malformed status view');
    if (!Array.isArray(body.snapshot.models))
        throw new Error('malformed status view');
...
export class ModelHealthController {
    ...
    constructor(deps = {}) {
        ...
        this.settings = createSnapshotStore({ ...DEFAULT_SETTINGS, ...deps.settingsSeed }, { persist: { name: SETTINGS_PERSIST_KEY } });
    ...
    sanitizePersisted() {
        ...
        if (positionValid && refreshValid)
            return;
        this.settings.update((draft) => {
            if (!positionValid)
                draft.position = DEFAULT_SETTINGS.position;
            if (!refreshValid)
                draft.refreshSeconds = DEFAULT_SETTINGS.refreshSeconds;
        });
    }
}
