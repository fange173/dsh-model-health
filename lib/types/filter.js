 ...
 */
export function sanitizeFilterInput(value) {
...
function sanitizeList(value) {
    ...
    for (const entry of value) {
        if (out.length >= MAX_FILTER_ENTRIES)
            break;
        if (typeof entry !== 'string' || entry.length === 0 || entry.length > MAX_FILTER_KEY_LENGTH)
            continue;
        if (seen.has(entry))
            continue;
        seen.add(entry);
...
export function isModelEnabled(filter, provider, model) {
    if (filter.disabledProviders?.includes(provider))
        return false;
 ...
 */
export function setProviderEnabled(filter, provider, enabled) {
    return enabled
 ...
 * @returns a new filter value.
 */
export function setModelEnabled(filter, provider, model, enabled) {
