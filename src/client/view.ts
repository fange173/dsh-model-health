 ...
 */
export function summarize(models: readonly ModelCheckResult[]): Aggregate {
  let ok = 0
  let latencySum = 0
  let latencyCount = 0
 ...
 */
export function aggregateTone(aggregate: Aggregate | null, hasError: boolean): HealthTone {
 ...
 */
export function resultTone(result: ModelCheckResult): HealthTone {
 ...
 */
export function groupByProvider(models: readonly ModelCheckResult[]): ProviderGroup[] {
  const groups: ProviderGroup[] = []
  let current: { provider: string; models: ModelCheckResult[] } | undefined
  for (const model of models) {
...
export function buildTrendSeries(
  ...
  const byModel = new Map<string, { meta: ModelCheckResult; points: TrendPoint[] }>()
 ...
 */
export function buildRoundSeries(
  history: readonly ModelHealthRound[],
): RoundPoint[] {
 ...
 */
export function recency(checkedAt: string | undefined, nowMs: number): Recency {
  if (checkedAt === undefined) return { kind: 'none' }
...
export function providerEnabled(filter: ModelHealthFilter, provider: string): boolean {
  return !(filter.disabledProviders?.includes(provider) ?? false)
}

/** Whether a provider/model is enabled under a denylist filter. */
export function modelEnabled(filter: ModelHealthFilter, provider: string, model: string): boolean {
 ...
 */
export function filterTrendSeries(series: readonly TrendSeries[], filter: ModelHealthFilter): TrendSeries[] {
...
export function toggleProvider(filter: ModelHealthFilter, provider: string): ModelHealthFilter {
 ...
 * first click.
 */
export function toggleModel(filter: ModelHealthFilter, provider: string, model: string): ModelHealthFilter {
...
function enableProvider(filter: ModelHealthFilter, provider: string): ModelHealthFilter {
  const disabled = filter.disabledProviders ?? []
